...
// Run once from the function dropdown (Run > repairOldDateTimeFormat),
// then redeploy. Earlier this session DATE_FORMAT switched from
// month-first (M/d/yyyy) to day-first (dd/MM/yyyy), and TIME_FORMAT
// switched from a bare 24-hour clock to 12-hour with AM/PM. Rows
// written before that switch are still stored in the old format and
// sit side by side with new rows — that's expected and harmless on
// its own. The real problem: a plain date string like "9/11/2026" is
// genuinely ambiguous (9 and 11 are both valid months), so trying to
// guess its format from the text alone can read it wrong — e.g. a row
// meaning September 11 getting misread as November 9 and sorting into
// the wrong day in the Visit Log.
//
// This removes the ambiguity instead of guessing at it: "date" is
// always written in the SAME action as "timeIn" (Visits) or "time"
// (Registrations/Pending) — check-in, submission, or approval — so
// that time value's format tells us for certain which format its
// paired date is really in. "timeOut" is written separately, later,
// at actual checkout, so it's judged and converted independently and
// is never used to decide (or touch) a row's date.
//
// Safe to run any number of times — a row already in the new format
// is left completely untouched.
function repairOldDateTimeFormat() {
  const OLD_TIME_RE = /^(\d{1,2}):(\d{2}):(\d{2})$/; // e.g. "10:37:23" — old, no AM/PM
  const OLD_DATE_RE = /^(\d{1,2})\/(\d{1,2})\/(\d{4})$/; // old was month-first: M/d/yyyy

  function convertOldTime(v) {
    const m = String(v).match(OLD_TIME_RE);
    if (!m) return null;
  ...
  function convertOldDate(v) {
    const m = String(v).match(OLD_DATE_RE);
    if (!m) return null;
  ...
  Object.keys(ACTIVITIES).forEach(key => {
    ...
    const dateCol = VISIT_HEADERS.indexOf("date") + 1;
    const timeInCol = VISIT_HEADERS.indexOf("timeIn") + 1;
    const timeOutCol = VISIT_HEADERS.indexOf("timeOut") + 1;

    const dates = sheet.getRange(2, dateCol, n, 1).getValues();
    const timeIns = sheet.getRange(2, timeInCol, n, 1).getValues();
    const timeOuts = sheet.getRange(2, timeOutCol, n, 1).getValues();

    let datesChanged = false, timeInsChanged = false, timeOutsChanged = false;

    for (let i = 0; i < n; i++) {
      const timeInVal = String(timeIns[i][0] || "");
      if (OLD_TIME_RE.test(timeInVal)) {
        const newTime = convertOldTime(timeInVal);
        const newDate = convertOldDate(String(dates[i][0] || ""));
        if (newTime !== null) { timeIns[i][0] = forceLiteralText(newTime); timeInsChanged = true; }
        if (newDate !== null) { dates[i][0] = forceLiteralText(newDate); datesChanged = true; }
        totalFixed++;
      }
      const timeOutVal = String(timeOuts[i][0] || "");
      if (timeOutVal && OLD_TIME_RE.test(timeOutVal)) {
        const newTimeOut = convertOldTime(timeOutVal);
        if (newTimeOut !== null) { timeOuts[i][0] = forceLiteralText(newTimeOut); timeOutsChanged = true; }
      }
    }

    if (datesChanged) sheet.getRange(2, dateCol, n, 1).setValues(dates);
    if (timeInsChanged) sheet.getRange(2, timeInCol, n, 1).setValues(timeIns);
    if (timeOutsChanged) sheet.getRange(2, timeOutCol, n, 1).setValues(timeOuts);
  });
  ...
  sheetsToRepair.forEach(({ name, headers }) => {
    ...
    const dates = sheet.getRange(2, dateCol, n, 1).getValues();
    const times = sheet.getRange(2, timeCol, n, 1).getValues();

    let datesChanged = false, timesChanged = false;

    for (let i = 0; i < n; i++) {
      const timeVal = String(times[i][0] || "");
      if (OLD_TIME_RE.test(timeVal)) {
        const newTime = convertOldTime(timeVal);
        const newDate = convertOldDate(String(dates[i][0] || ""));
        if (newTime !== null) { times[i][0] = forceLiteralText(newTime); timesChanged = true; }
        if (newDate !== null) { dates[i][0] = forceLiteralText(newDate); datesChanged = true; }
        totalFixed++;
      }
    }
    ...
    if (datesChanged) sheet.getRange(2, dateCol, n, 1).setValues(dates);
    if (timesChanged) sheet.getRange(2, timeCol, n, 1).setValues(times);
  });

  Logger.log(totalFixed + " row(s) converted from the old M/d/yyyy + 24-hour-time format to the " +
    "new dd/MM/yyyy + h:mm a format, across every Visits, Registrations, and Pending sheet. Each " +
    "row's own paired time value was used to decide its date format, so this is not a guess — even " +
    "previously-misread ambiguous dates (like a September 11th row that had been sorting under " +
    "November 9th) are now corrected.");
}
